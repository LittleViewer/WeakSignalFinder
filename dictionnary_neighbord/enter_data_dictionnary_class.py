...
class enter_data_dictionnary:
    ...
    def enter_last_run(self, job_id):
        self.cursor_database_.execute(f"INSERT INTO run VALUES ('{job_id}','{self.date_now}');")
        self.handle_dabase_.commit()

    def dectet_new_world(self, list_all_word_db,list_all_word):
        word_is_not_db = []
        word_exist = []
    ...
    def new_word_enter_db(self, word_is_not_db):
        ...
        for one_tick in range(len(word_is_not_db)):
            prepare_request = prepare_request + f"('{word_is_not_db[one_tick]}')"
            if one_tick != counter:
    ...
    def neighbor_enter_db(self, all_data):
        number_enter = 0
        prepare_request = "INSERT INTO dictionnary (central_word, position_, word_neighbor) VALUES "
            ...
            for one_part in all_data[one_line]:
                for one_word in all_data[one_line][one_part]:
                    ...
                    if len(check_is_exist) == 0:
                        ...
                        prepare_request = prepare_request + f" ('{one_line}','{one_part}','{one_word}'),"
    ...
    def pipe_enter_data(self, all_data):
        ...
        all_word_classify = self.dectet_new_world(set(word_list_in_db), list_all_word[0])
