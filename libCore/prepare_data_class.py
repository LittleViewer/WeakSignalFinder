...
class prepare_data:
        ...
        for one_index in structured_data:
            ...
            for one_article in structured_data[one_index]:
                for index_article in range(len(one_article)):
                    one_bloc = []
                    for one_word in re.sub(r'[^a-z0-9\u00C0-\u017E ]', ' ', self.obj_class_router["utils"]().remove_accent(one_article[index_article]).lower()).split(" "):
    ...
    def prepare_file_essential_word(self):
        ...
        tick = 0
        try :
            for one_link in [self.obj_class_router["config_toml_tool"]().key_return("path","file_model","prepare_data"), self.obj_class_router["config_toml_tool"]().key_return("path","file_stopword","prepare_data")]:
                handle_file =  self.obj_class_router["utils"]().file_open(self.obj_class_router["utils"]().absolute_link(one_link))
                if tick == 0:
            ...
            self.obj_class_router["log"]().pipe_log("The files could not be read.", "CRITICAL", "prepare_data() : prepare_file_essential_word()")
            self.obj_class_router["utils"]().error_with_reason("The files could not be read : prepare_file_essential_word()", True)
        return data_file
    ...
    def remove_no_essential_word(self, structured_data_sub_clean, id_model, new_dict):
        ...
        if id_model not in list_model_set:
            self.obj_class_router["log"]().pipe_log(f"A model is not present in the database, the id : {id_model}", "prepare_data() : remove_no_essential_word()", "WARN")
            return False     
        new_dict[id_model] = []
        self.obj_class_router["log"]().pipe_log(f"The '{list_model_name_support[id_model]}' spacy model was called!","INFO","prepare_data.remove_no_essential_word()")
    ...
    def pipe_prepare_data(self,data_brut,obj_database,job_id):
        ...
        if self.obj_class_router["utils"]().is_dict(data_brut) == False:
            self.obj_class_router["log"]().pipe_log("Input variable is not dict: prepare_data() : pipe_prepare_data()","CRITICAL","pipe_prepare_data()")
            self.obj_class_router["utils"]().error_with_reason("Input variable is not dict: pipe_prepare_data()", True) 
        ...
        self.obj_class_router["log"]().save_state(structured_data_sub_clean,"Brut")
        self.obj_class_router["prepare_request"]().insert_data_database(obj_database[0],obj_database[1],"saveData",["jobId","dateTime","type","data"],[[job_id,datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),"saveStateBrut",str(structured_data_sub_clean).replace("'",'"')]])
        ...
        self.obj_class_router["log"]().save_state(structured_data_clean_,"Clean")
        self.obj_class_router["prepare_request"]().insert_data_database(obj_database[0],obj_database[1],"saveData",["jobId","dateTime","type","data"],[[job_id,datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),"saveStateClean",str(structured_data_clean_).replace("'",'"')]])
