...
class prepare_request:
    ...
    def connect_dabase(self):
        try :
            link_to_database = self.obj_class_router["config_toml_tool"]().key_return("path","database_run_sqlite","database")
            if self.obj_class_router["utils"]().is_string(link_to_database) == True:
                handle_dabase = sqlite3.connect(self.obj_class_router["utils"]().absolute_link(link_to_database))
                cursor_database = handle_dabase.cursor()
            else :
                self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.connect_dabase()!",True)
        except:
    ...
    def get_data_table(self, cursor, table_name):
        if self.obj_class_router["utils"]().is_string(table_name) == False:
            self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.get_data_table_where_like()!",True)
        cursor.execute(f"SELECT * FROM {table_name}")
        return cursor.fetchall()
    
    def get_data_table_where_like(self, cursor, table_name, column_name, like_value):
        if self.obj_class_router["utils"]().is_string(table_name) == False or self.obj_class_router["utils"]().is_string(column_name) == False or self.obj_class_router["utils"]().is_string(like_value) == False:
            self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.get_all_data_table()!",True)
    ...
    def insert_data_database(self, cursor, handle_database, table_name, column_name, values):
        if self.obj_class_router["utils"]().is_string(table_name) != True or self.obj_class_router["utils"]().is_list(column_name) != True:
            self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.insert_data_database()#45!",True)
        temp_values = []
        ...
        for one_row_values in range(tick_values_all_row):
            ...
            if self.obj_class_router["utils"]().is_list(one_row_content) != True:
                self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.insert_data_database()#74!",True)
        ...
        try :
            ...
            if sqlite3.IntegrityError == type(error):
                self.obj_class_router["utils"]().error_with_reason("There is an error with an integrity constraint, such that data already entered is entered again!")
            else:
                self.obj_class_router["utils"]().error_with_reason("An unexpected error occurred while inserting data into the database!",True)
        
    def delete_data(self, cursor, handle_database, table_name, column_name, data):
        try:
            if self.obj_class_router["utils"]().is_string(table_name) != True or self.obj_class_router["utils"]().is_string(column_name) != True or self.obj_class_router["utils"]().is_string(data) != True:
                self.obj_class_router["utils"]().error_with_reason("The expected formats are not provided as input in prepare_request.delete_data()!",True)
            prepare_request = f"DELETE FROM {table_name} WHERE {column_name} LIKE '{data}';"
            cursor.execute(prepare_request)
            handle_database.commit()
        except:
            self.obj_class_router["utils"]().error_with_reason("An unexpected error occurs when trying to delete data!",True)
